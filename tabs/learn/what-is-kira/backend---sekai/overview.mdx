...
Unlike most existing base layers, SEKAI is not a monolithic chain application. Instead, it depends on multiple modular off-chain components to enable parallel scaling of the execution, storage, and networking capabilities of the network. Those other components are [INTERX (Content Availability Layer)](/learn/what-is-kira/middleware---interx/overview) and [RYOKAI (Smart Resource Orchestrator)](/learn/what-is-kira/orchestrator---ryokai).
