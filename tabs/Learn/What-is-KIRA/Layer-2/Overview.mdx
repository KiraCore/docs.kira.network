...
Any users can permissionlessly become Fishermen, i.e. if they have a vested interest in this RollApp. They are not subject to approval by controllers but must submit a bond in the RollApp's [liquidity pool tokens](/learn/a6a660a6-3e5c-4a53-9ec3-14aa07f00351). The bond amount is set via governance, defaulting to 0.1% of the pool’s total LP tokens circulating supply. In summary, Consensus nodes can opt-in to run as Executors or Verifiers, pending approval. Other users can freely become Fishermen verifiers by providing a bond.
